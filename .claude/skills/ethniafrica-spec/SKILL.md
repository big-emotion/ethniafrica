---
name: ethniafrica-spec
description: Spec maintainer for EthniAfrica. Reads Confluence (Requirements / Decisions / Architecture) as the source of truth, helps you investigate a problem, structures the reflection, and produces drafts — Pending REQ/DEC/ARCH sections on Confluence + matching Jira tickets in ETNI with the project template. Append-only on Confluence, Pending only. Use when you face a bug, want to add a feature, or need to refine an idea.
metadata:
  author: Big Emotion
  version: "1.0.0"
---
...
Maintain the EthniAfrica spec tree on Confluence. Given a free-text problem (bug, feature, refinement idea), read Confluence (`Requirements / Decisions / Architecture`) as the canonical source of truth, structure the reflection, propose `Pending` REQ/DEC/ARCH sections plus matching Jira tickets in `ETNI`, then — after explicit confirmation — publish Confluence first, Jira second, and close the link loop both ways.
...
- Do **not** activate when:
  - The user pastes a Jira ticket URL or key — that path belongs to `ethniafrica-ticket`.
  ...
  - The user asks to bootstrap or repopulate the spec tree — that is `/ethniafrica-bootstrap-confluence` and runs only once.
...
6. **Template available** — `docs/templates/jira-ticket-template.md` exists. If absent, stop — the Jira tickets cannot be filled.
...
| Editorial-only Jira refinement (no REQ/DEC/ARCH change)              | no ticket — reference the existing one |
...
4. The ticket title is concise and business-readable; it carries the primary touched ID in parentheses (e.g. `Search: keep filters across page reloads (REQ-042)`).
...
Jira tickets — M total (issue type from granularity table)
  • [Story] <business-readable title> (REQ-042)
      Confluence impact:
...
2. Create Stories next, each carrying `customfield_<epic_link>` or `parent.key = <epic_key>` per the project's epic-link convention (the value comes from the template's frontmatter or from the issue type's create metadata if unknown).
...
```
Links → Jira: ETNI-1234, ETNI-1235
```
...
Jira (new tickets):
  • ETNI-1234 [Story] <business-readable title> (REQ-042)
      → https://big-emotion.atlassian.net/browse/ETNI-1234
...
The skill is single-writer per invocation but the canonical Confluence pages can change between two consecutive reads, because other writers (humans editing in the UI, the bootstrap skill on a one-shot replay attempt, or another instance of this skill in a parallel session) may write concurrently. Two mechanisms handle this:
...
| Condition                                                                 | Action                                                                                     |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `docs/.confluence-bootstrap-complete` missing                             | Stop. Tell user to run `/ethniafrica-bootstrap-confluence` first.                          |
...
| Jira project `ETNI` not visible to the current user                       | Stop. The user lacks access.                                                               |
...
- An alignment agent that classifies each diff as `aligned / advance / drift`. Future milestone.
- A mirror `specs/SPEC.md` regenerated from Confluence. Future milestone.
- A hard CI gate that fails a PR on drift. Future milestone.
- Multi-project Jira routing. This skill only targets the project whose key is in `config.json` (`ETNI`).
...
- Editing existing Jira tickets. That belongs to `ethniafrica-ticket` (for tickets it owns end-to-end) or to the human via the Jira UI.

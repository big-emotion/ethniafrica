...
description: One-shot bootstrap of the Confluence spec tree (Requirements / Decisions / Architecture / Obsolete) for EthniAfrica. Creates the four skeleton subpages under the project's engineering root page in the dedicated ETHNIAFRIC space, records their IDs in the spec config, writes the sentinel, then refuses second runs. Use only when initializing the spec system, never for ongoing maintenance.
metadata:
  author: Big Emotion
  version: "1.0.0"
---
...
This skill publishes the four pages as **skeletons**: every REQ/DEC/ARCH section arrives through `/ethniafrica-spec` at status `Pending`. It is the entry point of the Confluence-as-source-of-truth system, **not** the ongoing maintenance loop — that belongs to `/ethniafrica-spec`.
...
- User invokes `/ethniafrica-bootstrap-confluence` (no arguments).
...
1. **In the repo root** — `package.json` `.name` is `ethniafrica`. If not, stop.
...
4. **Config present and parseable** — `docs/confluence-spec/config.json` must exist and contain non-null `cloudId`, `siteUrl`, `spaceKey`, `engineeringRootPageId`. The four `*PageId` fields may be `null` (fresh config — this is their current state) or hold ids from a previous tree (deliberate re-init); this skill overwrites them in Phase 4 either way. Do not hardcode these values inside this skill; always read them at runtime. For orientation only, the config currently points at space `ETHNIAFRIC` and engineering root page `174948648` — if the file says otherwise, the file wins.
...
6. **No existing spec sub-tree** — `mcp__atlassian__getConfluencePageDescendants` on `engineeringRootPageId` must NOT contain a child titled `Requirements`, `Decisions`, `Architecture`, or `Obsolete`. If any exists, refuse and tell the user to remove it manually (or reconcile the config by hand) — a half-built tree is a human decision, not something to merge into silently.

After all preconditions pass, **before doing anything else**, print verbatim the discipline phrase:

```
This is the one-shot bootstrap. After publish, spec intent lives on Confluence and flows from Confluence to code only. Ongoing changes go through /ethniafrica-spec.
```

## Page-title convention — bare titles

The Confluence space `ETHNIAFRIC` is **dedicated to EthniAfrica**, not shared with another project's spec tree. Confluence enforces title uniqueness per space, and nothing else in this space owns these titles, so the four pages use **bare titles**:

`Requirements` · `Decisions` · `Architecture` · `Obsolete`

No `EthniAfrica — ` prefix. This decision is made once, at install time, and must stay consistent with what `/ethniafrica-spec` expects — that skill resolves pages by their `*PageId` in `docs/confluence-spec/config.json`, never by title, so a later human rename of a page does not break it. The titles matter here only for the Phase 0 collision guard and for human readability.
...
1. Check `docs/.confluence-bootstrap-complete`. If present, read it (it is small JSON), print its `timestamp` and recorded page IDs, and **abort with the message**: "Bootstrap already completed at <ts>. This skill is one-shot. Use /ethniafrica-spec for ongoing changes."
...
4. `getConfluencePage(cloudId, pageId=engineeringRootPageId)` — confirm the root page exists in the configured space. If 404, abort.
5. `getConfluencePageDescendants(cloudId, pageId=engineeringRootPageId)` — scan children titles. If any child is titled `Requirements`, `Decisions`, `Architecture`, or `Obsolete`, abort.
6. Print the discipline phrase verbatim (see Preconditions).

### Phase 1 — Compose the four skeleton bodies (in memory)

Each page opens with one sentence stating its contract. No REQ/DEC/ARCH sections are published at bootstrap — the tree starts empty and `/ethniafrica-spec` appends the first `Pending` sections later.

- **Requirements** — "This page holds the canonical REQ-NNN requirements of EthniAfrica. Sections are appended by /ethniafrica-spec at status Pending; humans transition them via the Status macro."
- **Decisions** — "This page holds the canonical DEC-NNN decisions of EthniAfrica. Sections are appended by /ethniafrica-spec at status Pending; humans transition them via the Status macro."
- **Architecture** — "This page holds the canonical ARCH-NNN architecture contracts of EthniAfrica. Sections are appended by /ethniafrica-spec at status Pending; humans transition them via the Status macro."
- **Obsolete** — "This page lists intents that have been retired. They are kept for historical context; the current state lives in Requirements / Decisions / Architecture."

Bodies are written in **English** (repo docs-language rule), even though the product UI is French.

**No Status macros at bootstrap.** There is no content to mark. `Pending` is exclusively produced by `/ethniafrica-spec`; `Obsolete` entries appear only when a RETIRE is published. If you find yourself about to render a Status macro here, you have made a category error — stop.
...
1. `createConfluencePage(cloudId, spaceKey, parentId=engineeringRootPageId, title="Requirements", body=<skeleton>)` → `requirementsPageId`
2. Same for `Decisions` → `decisionsPageId`
3. Same for `Architecture` → `architecturePageId`
4. Same for `Obsolete` → `obsoletePageId`
...
Persist the new page IDs so `/ethniafrica-spec` can find them. Use the Edit tool to set exactly these four fields:
...
Do **not** touch any other field. Preserve the file's existing 2-space indentation and field order (user `CLAUDE.md` JSON rule).
...
3. Create a new branch and commit (do NOT commit on `recette` or `main`):

   ```bash
   git checkout -b bootstrap/confluence-spec-init
   git commit -m "chore(confluence): bootstrap ethniafrica spec tree (one-shot)"
   ```
...
4. **Do NOT push.** Print the final report:
   ...
   From now on, spec intent flows from Confluence to code only. Use /ethniafrica-spec for ongoing changes.
   ```
...
| Engineering root already has a child titled `Requirements` / `Decisions` / `Architecture` / `Obsolete`                   | Abort. Manual cleanup or manual config reconciliation required.                                    |
...
- `Pending` sections of any kind. Bootstrap publishes empty skeletons only. `Pending` is the exclusive output of `/ethniafrica-spec`.
- Migrating existing repo docs (`docs/architecture.md`, `docs/api-contracts.md`, `docs/adr/*`, `CLAUDE.md`) into the tree. If the team later wants them speced, each one goes through `/ethniafrica-spec` as a reviewed draft — never as a bulk import.
- Ongoing maintenance of the Confluence tree (adding new REQs, updating DECs as code evolves). That belongs to `/ethniafrica-spec`.
- Any Jira write — assigning tickets, creating issues, transitioning. This skill never touches Jira, including the `ETNI` project.
...
- **No Status macros**: the only legitimate bootstrap output is four skeleton pages with no macros. `Pending` belongs to `/ethniafrica-spec`.

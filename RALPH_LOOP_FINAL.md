# Ralph Loop Final - Migration v2 Complète

Ce fichier contient une **commande unique** pour finaliser la migration API v1 → v2.
...
| #   | Tâche             | Description                                                                  |
| --- | ----------------- | ---------------------------------------------------------------------------- |
| 1   | HierarchicalNav   | Composant de navigation arborescente (Famille → Peuple → Pays)               |
| 2   | Page Recherche    | Page dédiée `/[lang]/recherche` avec filtres et pagination                   |
| 3   | DemographicsChart | Visualisation des données démographiques                                     |
| 4   | Supprimer API v1  | Supprimer `/api/stats`, `/api/regions`, `/api/ethnicities`, `/api/countries` |
| 5   | Tests afrikLoader | Tests unitaires pour `src/lib/afrikLoader.ts`                                |
| 6   | Tests composants  | Tests pour les composants v2 principaux                                      |

---

## Commande Ralph Loop Unique

```bash
/ralph-loop "Complete the API v1 to v2 migration for EthniAfrica. Execute the following tasks IN ORDER, committing after each task:
...
Create src/lib/__tests__/afrikLoader.test.ts:
...
When ALL tasks are complete and validation passes, output: <promise>MIGRATION_V2_FINAL_COMPLETE</promise>" --max-iterations 30 --completion-promise "MIGRATION_V2_FINAL_COMPLETE"
```
...
git checkout feat/afrik-loader-and-docs
...
# Voir le travail fait
git status
git diff

# Commiter le travail partiel si nécessaire
git add -A && git commit -m "wip: partial migration work"
```
...
# Si tout est OK, merge vers main
git checkout main
git merge feat/afrik-loader-and-docs
...
- [ ] Task 1: HierarchicalNav créé
- [ ] Task 2: Page recherche dédiée créée
- [ ] Task 3: DemographicsChart créé
- [ ] Task 4: API v1 supprimée
- [ ] Task 5: Tests afrikLoader passent
- [ ] Task 6: Tests composants passent

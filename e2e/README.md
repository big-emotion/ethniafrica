...
`E2E_LOCALE` defaults to the site's default locale; CI runs a matrix over both
(`e2e.yml`). A spec that reads UI copy skips itself outside `fr` until that
copy is translated, so the English leg covers the route, CSP and axe specs and
reports the rest as skipped — see `e2e/support/locale.ts`.

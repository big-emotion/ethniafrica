...
- [x] [Review][Patch] Restore the page-to-service boundary with a narrow, serializable country-atlas brief containing only the approved population, reference year, and language fields. [`src/api/v2/services/countryService.ts`, `src/app/[lang]/explorer/pays/[slug]/page.tsx`]
- [x] [Review][Patch] Preserve corpus silence without exposing `0 peuples documentés` or an unknown placeholder when the count is absent. [`src/components/country/countryTargetFacts.tsx`]
- [x] [Review][Patch] Render population only when paired with a valid positive integer reference year, and keep the year explicitly labelled. [`src/components/country/countryTargetFacts.tsx`]
- [x] [Review][Patch] Enforce the compact brief contract before the client boundary: deduplicate and cap languages server-side, require the brief input, and derive the current-country fallback from the current fiche. [`src/api/v2/services/countryService.ts`, `src/components/country/countryTargetFacts.tsx`]
- [x] [Review][Patch] Add page/service coverage and update every country-service mock, including the full fiche contract and legacy `majorPeoples` fallback. [`src/api/v2/services/__tests__/countryService.test.ts`, `src/app/[lang]/explorer/pays/__tests__/page.test.tsx`, `src/app/[lang]/__tests__/fiche-vivante.test.tsx`, `src/components/country/__tests__/countryTargetFacts.test.tsx`]
- [ ] [Review][Patch] Format the specification and run the complete required CI gate before merging. [`_bmad-output/implementation-artifacts/spec-country-summary.md`]

...
function centroidOf(iso, GEO) {
  ...
  const sum = ring.reduce(
    ([alon, alat], [lon, lat]) => [alon + lon, alat + lat],
    [0, 0]
  );
...
function createGlobe({ canvas, GEO, AFRICA_PATH, drawOverlay }) {
  ...
  function buildBaseTexture() {
    ...
    for (let lon = -180; lon <= 180; lon += 15) {
      ctx.strokeStyle =
        lon % 90 === 0 ? PALETTE.graticuleMajor : PALETTE.graticule;
      ctx.beginPath();
    ...
    ctx.save();
    const sx =
      (lonToX(AFRICA_BOUNDS.lonMax) - lonToX(AFRICA_BOUNDS.lonMin)) /
      BASEMAP.width;
    const sy =
      (latToY(AFRICA_BOUNDS.latMin) - latToY(AFRICA_BOUNDS.latMax)) /
      BASEMAP.height;
  ...
  overCanvas.width = OVER_W;
  overCanvas.height = OVER_H;
  ...
  canvas.addEventListener("pointermove", (e) => {
    ...
    rotXTarget = Math.max(
      -1.0,
      Math.min(1.0, rotXTarget + (e.clientY - lastY) * 0.004)
    );
    lastX = e.clientX;
    lastY = e.clientY;
  });
  ...
  return {
    ...
    flyTo(lon, lat, span) {
      ...
      dy =
        ((((dy + Math.PI) % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI)) -
        Math.PI;
      rotYTarget += dy;
      rotXTarget = t.x;
      ...
      zoomTarget = span
        ? Math.max(0.62, Math.min(0.95, 0.46 + span / 78))
        : 0.78;
      morphTarget = 1;
      reveal();
    },

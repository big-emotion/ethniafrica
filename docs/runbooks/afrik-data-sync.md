...
and `/en/atlas/ethnonyms` once bilingual publication is enabled) (ETNI-1821).
...
Finally, load the published migrations atlas (`/fr/dossiers/migrations`; its `/en` counterpart
is reachable only in a bilingual mode) as an anonymous visitor. If it renders an empty state while the

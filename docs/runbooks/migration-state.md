...
| `068_unified_search_surface.sql`              | pending — applies on merge via `migrate-recette.yml`          | pending — apply by hand                                |

> **REQ-002 (ETNI-1707).** `068` is the blocking SQL contract behind cross-kind search. It adds
...
>
> `068` also gives two kinds an RPC for the first time: `afrik_search_language_families`, which
...
> as `050`); `068` is the next free version. Until production carries `068`, quiz questions and
> the family ladder rank on recette and not there, and a merged result list on production keeps
> ordering by kind.
